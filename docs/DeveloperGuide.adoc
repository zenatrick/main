...
[PackingItemList/FixedExpenseList/TransportList] against the hard-coded database in `Storage` and return a
`CommandResult` object which consists of a `MESSAGE_SUCCESS` back to `LogicManager`
4.	After receiving the `CommandResult` from `CheckStatusCommand`, `LogicManager` will then proceed to
