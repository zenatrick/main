...
**E.T. (Easy Travel)** is a desktop travel planning application made for travellers who want to plan for their trip. It focuses on the <<cli,__Command Line Interface (CLI)__>> while providing users with a simple and clean <<gui, __Graphical User Interface (GUI)__>>. Thus, the main interaction with **E.T.** will be done through commands.
...
NOTE: The lifelines for the `DeleteItemCommandParser` and `DeleteItemCommand` should end at the destroy marker (X). However, due to a limitation of PlantUML, the lifelines reache the end of the diagram.
...
As described in <<logic-component>> <<Design-Logic>>, after the user enters a command, the `EasyTravelParser` will generate an `XYZCommandParser` which parses the user input parameters and generate an executable `XYZCommand` that performs an operation on the list.
...
. Test case: `addacc name/JW Marriott Hotel loc/KL startday/3 endday/4 remark/Check-in at 3pm.`
.. Expected: Adds an accommodation with name of `JW Marriott Hotel` location of `KL`,  startday of `3`, endday of `4` and remark of `Check-in at 3pm`.
...
. Test case: `addtransport mode/plane startloc/Singapore endloc/Japan starttime/04-10-2020 17:00 endtime/06-10-2020 00:00`
.. Expected: Adds an transportation with mode `plane`, startloc as `Singapore`, endloc as `Japan`, starttime as `04-10-2020 17:00` and endtime as `06-10-2020 00:00`.
