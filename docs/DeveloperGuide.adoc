...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `XYZTabPanel`, `DashboardPanel` etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
The sequence diagram below shows the the execution of `SetTripCommand`:
...
* **Alternative:** A trip can be created after the use of any other commands
** Pros: Gives the user the freedom to use commands before the trip is set, such as the budgeting and packing list.
** Cons: Defeats the purpose of planning the trip from top down, starting from the trip being set first.
// end::Trip Management[]
...
The `addpreset` command requires the user to include the `category` of the preset they want to add into the packing list.
For example, `addpreset swimming` means that the preset called `swimming` will be be added into the user's packing list.
...
These objects are used to create the `AddPresetCommand` object that will execute to create a packing list item.
...
2. A `CommandResult` object which consists of a `MESSAGE_SUCCESS` is sent back to `LogicManager` if there is a successful addition of the items into the packing list.
...
* **Current Choice:** Category can only be `accommodation`, `activities`, `transport` or `others`
** **Pros:** Keeps the implementation simple, and since a pie chart will be shown, the pie chart will be kept simple and clean as well.
** **Cons:** Restricts the user on the categories they can use, prevents them from customising the category to suit their expenses.

* **Alternative:** Category can be any user input
** **Pros:** Not restricted to a fixed category, gives the user freedom to choose and customise their fixed expenses.
** **Cons:** The pie chart shown will look messy, not visually pleasing to the user.
// end::fixedexpense[]
