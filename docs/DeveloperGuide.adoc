...
.Sequence diagram that describes the creation of the `ScheduleActivityCommand` object.
image::ScheduleAcitivityCommandCreation.png[]
...
1. `ModelManager` retrieves the list of days from the `Trip` and the list of activities from the `UniqueList` that stores `Activity` objects.
2. The `activityIndex` is used to obtain the target `Activity` from the activity list and and `dayIndex` is used to obtain the target day's `DaySchedule` from the list of days.
3. `Activity` is converted to a `Schedulee` object using the `startTime` from the `ScheduleActivityCommand`, the `title` and `duration` from the target `Activity`.
4. The `Schedulee` object is added to the target day's `DaySchedule`.
5. The list of `Schedulee` is sorted according to the `startTime`.
...
image::ScheduleActivityCommandExecution.png[]
